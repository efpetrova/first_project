...
@app.route('/')
def main():
    return render_template('index.html', departures=departures, tours=list(tours.values())[0:6])


@app.route('/departures/<departure>/')
def departure(departure):
    departure_title = departures[departure]
    tours_departure = {key: value for key, value in tours.items() if value["departure"] == departure}
    return render_template('departure.html', departure=list(departures.keys()), departures=departures, tours=tours_departure,
